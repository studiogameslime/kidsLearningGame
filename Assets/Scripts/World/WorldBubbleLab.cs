...
/// <summary>
/// Interactive Bubble Lab icon in the World scene.
/// Playful idle animations (bounce, sway, bubbles) attract attention.
/// Tapping opens the Bubble Lab sandbox scene.
/// </summary>
public class WorldBubbleLab : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (!isIdleAnimating && idleTimer >= nextIdleTime)
        {
            ...
            switch (action)
            {
                ...
                case 2: StartCoroutine(IdleBubbleParticles()); break;
            }
        }
    }
    ...
    private IEnumerator IdleBubbleParticles()
    {
        isIdleAnimating = true;

        Color[] colors = {
            new Color(0.95f, 0.25f, 0.25f, 0.6f),
            new Color(0.25f, 0.55f, 0.95f, 0.6f),
            new Color(0.95f, 0.85f, 0.15f, 0.6f),
            new Color(0.25f, 0.85f, 0.40f, 0.6f),
            new Color(0.70f, 0.30f, 0.85f, 0.6f),
        };

        int count = Random.Range(3, 5);
        for (int i = 0; i < count; i++)
        {
            SpawnIdleBubble(colors[Random.Range(0, colors.Length)]);
    ...
    private void SpawnIdleBubble(Color color)
    {
        var go = new GameObject("LabBubble");
        ...
        float xOff = Random.Range(-35f, 35f);
        ...
        StartCoroutine(AnimateIdleBubble(bubbleRT, img));
    }

    private IEnumerator AnimateIdleBubble(RectTransform bubbleRT, Image img)
    {
        float dur = Random.Range(0.8f, 1.3f);
        float elapsed = 0f;
        ...
        while (elapsed < dur)
        {
            elapsed += Time.deltaTime;
            float t = elapsed / dur;
            float y = start.y + 55f * t;
            float x = start.x + Mathf.Sin(swayPhase + t * Mathf.PI * 3f) * 10f;
            bubbleRT.anchoredPosition = new Vector2(x, y);
            img.color = new Color(img.color.r, img.color.g, img.color.b, img.color.a * (1f - t));
            yield return null;
        }
    ...
    private IEnumerator TapSequence()
    {
        ...
        NavigationManager.GoToBubbleLab();
    }
}
